<script lang="ts">
    ...
    import { getLinkById, updateLink, checkDuplicateUrl, checkPublicDuplicateUrl } from '$lib/services/link-service';
    ...
    async function loadLink() {
        ...
        try {
            ...
            // Save the original URL to check if it changed during update
            originalUrl = link.url;
            originalVisibility = link.visibility;
            
            // Convert tags array to comma-separated string
            const tagsString = link.tags ? link.tags.join(', ') : '';
    ...
    async function checkUrlDuplicate() {
        // Only check for duplicates if the URL has changed from the original
        if (!formData.url || !validateUrl(formData.url) || formData.url === originalUrl) {
            duplicateUrlError = '';
    ...
    function handleVisibilityChange() {
        ...
        if (formData.visibility === 'public' && formData.url && validateUrl(formData.url) && formData.url !== originalUrl) {
            clearTimeout(urlChangeTimeout);
            urlChangeTimeout = setTimeout(checkUrlDuplicate, 100);
        } else if (formData.visibility === 'public' && formData.visibility !== originalVisibility) {
            // If the link is staying at the same URL but going from private to public
            clearTimeout(urlChangeTimeout);
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            // If changing to public, do one final check for public duplicates
            if (formData.visibility === 'public' && 
                (formData.visibility !== originalVisibility || formData.url !== originalUrl)) {
                const isPublicDuplicate = await checkPublicDuplicateUrl(formData.url);
            ...
            error = err.message || 'An error occurred while updating the link';
        ...
        {:else}
            <form on:submit|preventDefault={handleSubmit}>
                ...
                <div class="form-group">
                    ...
                    <select id="visibility" bind:value={formData.visibility}>
